...
![TCP 연결/종료 및 연속 요청 캡처](images/image.png)

앞선 10.3절의 캡처들이 Keep-Alive 도입 이전(요청마다 handshake·종료가 개별 발생)의 결과였다면, 이 캡처는 현재 코드(Keep-Alive 적용 버전)로 다시 캡처한 화면으로, 하나의 TCP 연결 위에서 여러 요청이 어떻게 이어지고 언제 종료되는지를 보여준다.

- **12~14번 패킷**: `SYN` → `SYN, ACK` → `ACK` 3-way handshake. `TCP Connect` 시점에 **딱 한 번만** 발생한다.
- **15~18번 패킷**: `GET /users HTTP/1.1` 요청과 `HTTP/1.1 200 OK` 응답(JSON 바디 포함, Len=1338). 별도의 handshake 없이 기존 연결 위에서 바로 오간다.
- **19~22번 패킷**: 이어서 입력한 `HEAD /users HTTP/1.1` 요청과 `HTTP/1.1 200 OK` 응답. 15~18번과 **같은 `tcp.stream`**이며, 새 연결이 맺어지지 않았다는 점에서 GET 응답과 HEAD 응답이 하나의 세션을 공유함을 확인할 수 있다. 또한 응답 패킷의 크기(Len=138)가 GET 응답(Len=1338)보다 훨씬 작은데, 이는 HEAD가 바디 없이 헤더만 반환하기 때문이다(4.3절, 6.3절 참고).
- **23~26번 패킷**: `FIN, ACK`(클라이언트→서버) → `FIN, ACK`(서버→클라이언트) → `ACK`로 이어지는 연결 종료 시퀀스. 이 종료는 GET·HEAD 두 요청·응답 사이에는 전혀 나타나지 않고, 사용자가 `q`를 입력해 `clientSocket.close()`를 호출한 시점에 **오직 한 번**만 발생한다.

즉 두 개 이상의 서로 다른 HTTP 요청(GET, HEAD)이 handshake 1회·종료 1회를 공유하는 동일한 TCP 연결 안에서 순차적으로 처리되었음이 패킷 레벨에서 그대로 드러나며, 이는 3.3절에서 설명한 Keep-Alive(지속 연결) 구조가 실제로 동작함을 증명하는 근거가 된다.

---

## 11. 과제 구현 중 어려웠던 점

### 11.1 Persistent Connection 전환에 따른 메시지 프레이밍 문제

처음에는 요청마다 새 TCP 연결을 맺고 응답 직후 곧바로 닫는 구조로 구현했는데, 이때는 `recv()` 한 번으로 응답을 통째로 읽어도 문제가 없었다. 서버가 응답을 보낸 뒤 바로 연결을 닫아주었기 때문에 "연결 종료 = 응답 끝"이라는 신호를 그대로 이용할 수 있었기 때문이다. 하지만 여러 요청을 같은 소켓으로 처리하는 Keep-Alive 구조로 바꾸자 이 가정이 깨졌다 — 연결이 계속 열려 있으므로 소켓이 닫히는 순간을 기준으로 응답의 끝을 판단할 수 없게 되었다. 결국 `Content-Length` 헤더 값만큼 바이트를 다 받을 때까지 `recv()`를 반복하도록 클라이언트(7.3절 `recv_response()`)와 서버(6.2절) 양쪽의 수신 로직을 모두 다시 작성해야 했다.

### 11.2 `Expect: 100-continue` 흐름의 동기화

클라이언트와 서버가 바디 전송 시점을 어긋나지 않게 맞추는 것이 까다로웠다. 클라이언트가 헤더만 먼저 보내고 `100 Continue`를 기다리는 동안, 서버가 `Expect` 헤더를 확인하지 않고 곧바로 바디를 기다려버리면 양쪽이 서로 상대의 전송을 기다리는 교착 상태에 빠질 수 있었다. 서버가 헤더 파싱 단계에서 `Expect: 100-continue` 유무를 먼저 판별해 `100 Continue`를 응답한 뒤에야 바디를 수신하도록 순서를 명확히 하고 나서야 4.5절의 흐름이 안정적으로 재현되었다.
